...
class WindowMgr:
    ...
    def capture_window(self, mode=0):
        ...
        img = Image.frombuffer('RGBX', (bmpinfo['bmWidth'], bmpinfo['bmHeight']), bmpstr, 'raw', 'BGRX', 0, 1).convert('RGB')
        return img
...
class CaptureViewer(tk.Frame):
    ...
    def __init__(self, parent=None):
        ...
        names = tuple(map(lambda x:x[1].decode('gbk'), self.win_mgr.wins))
        ...
        self.mode_button = tk.Button(parent, text='Margins', command=self.toggle_mode)
        self.mode_button.grid(column=1, row=0)

        ############################
        # Button: start preview window
        self.preview_button = tk.Button(parent, text='Preview', command=self.start_preview)
        self.preview_button.grid(column=2, row=0)

        ############################
        # Button: save window
        self.save_button = tk.Button(parent, text='Save', command=self.save_win)
        self.save_button.grid(column=3, row=0)

        ############################
        # Button: quit program
        self.quit_button = tk.Button(parent, text='Quit', command=self.quit)
    ...
    def start_preview(self):
        ...
        self.win_mgr.set_win(idx)
        self.update_img_label()

    def update_img_label(self):
        img = self.win_mgr.capture_window(self.mode)
        if img:
            tk_img = ImageTk.PhotoImage(img)
    ...
    def save_win(self):
        self.win_mgr.save_window('capture_%s.bmp' % str(self.count), self.mode)
        self.count += 1
